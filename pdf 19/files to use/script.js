...
questionB(); - will return bob, promise b, sponge, timeout b. because bob is main, promise b is from type 
               micro-thread so it will go next, then will come the timeout's because they're from type callback.
questionC(); - bob, promise b, promise c, sponge, timeout b, timeout c. same idea as before but the twist is that we'd
               ...
               it run in the micro-thread part.
questionD(); - bob, sponge, bob, promise b, promise c, pants, sponge, timeout b, timeout c, square.
questionE(); -  bob,
                sponge,
                bob,
                promise b,
                ...
                timeout b,
                timeout c.
                it follows the same principle, first the plain log (main) then the promises (microthread), then the timeout (callback).
*/